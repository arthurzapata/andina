...
$query_mos_doc = "SELECT * FROM cmd_imagen WHERE img_id = $key";
...
$pro_id= $row_mos_doc['not_id'];
	...
	if ($row_mos_doc['img_activo'] == 0) 

		$es = 1;

	elseif($row_mos_doc['img_activo'] == 1)

		$es = 0;

	

  $deleteSQL = sprintf("update cmd_imagen set img_activo = $es where img_id=%s",
